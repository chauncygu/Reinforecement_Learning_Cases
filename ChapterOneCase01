'''
the origin code is coded by Guoxian

Modified by chauncygu
...
class KB_Game:
    ...
    def step(self, a):
        r = 0
        if a == 1:
            r = np.random.normal(18,1)
        if a == 2:
            r = np.random.normal(2,1)
        if a == 3:
            r = np.random.normal(1.1,1)
        return r
...
'''Through the experiments, I have found that when the environments are different, the evaluation results will be different, which means that the policy is hard to be evaluated. For example, when the r function is different, the result is different.'''
